...
--1) % WildCard : It represents 0 or more characters 
--Select employee names which start with 'E'
SELECT name FROM employees 
WHERE name = 'Eddie Murphy';--HARD CODING

SELECT name FROM employees 
WHERE name LIKE 'E%';--IT IS DYNAMIC AND RECOMMENDED
...
--Be careful about using AND operator vecause of the logic rules
...
--1.WAY:It is DRY(Don't repeat yourself) It is repetation and it is not good
SELECT word FROM words 
WHERE word LIKE 'ho%t' OR word LIKE 'hi%t' OR word LIKE 'ha%t' ;
--2.WAY:RECOMMENDED
SELECT word FROM words
...
SELECT word FROM words
WHERE word~ 'h[a-e](.*)t';--[a-e] --> [abcde]

--Select words whose first character is 's' or 'a' or 'y'
SELECT word FROM words
WHERE word~ '^[say](.*)';--> ^ means first character when you use it before of the bracets

--Select words whose last character is 'm' or 'a' or 'f'
SELECT word FROM words
WHERE word~ '(.*)[maf]$';--> $ means last character when you use it after of the bracets
...
SELECT word FROM words
WHERE word~'a';
--2 ways are same (.*)a(.*) and 'a' is same  and 'a' means is it contains 'a'?

--Select words which have characters from 'd' to 't' at the beginning followed by any character then 'l'
SELECT word FROM words
WHERE word~ '^[d-t].l';--. means a single character 
...
--2.WAY:
SELECT name FROM workers
WHERE name~'^[^E](.*)[^y]$';--> If you use ^ inside the bracets it means 'not' or different from
