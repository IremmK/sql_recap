...
--ORDER BY: To put the records in ascening order or descending order we use ORDER BY
--ORDER BY can be used jst with SELECT.

--Put the records in ascending order by using number_of_letters
SELECT * FROM words 
ORDER BY number_of_letters ;--When you type after pharantesis ASC you will get same output
--SELECT * FROM words ORDER BY (number_of_letters) ASC
--ASC is optional it is ascending order as default

--Put the records in descending order by using word
SELECT * FROM words 
ORDER BY word DESC ;--> DESC is not optional if you don't type descending order it will be in ascending order
...
SELECT state, COUNT (state) AS "number of employee" FROM workers 
GROUP BY state 
...
SELECT company, MAX (salary) AS max_salary_per_company, MIN (salary) AS min_salary_per_company
FROM workers
GROUP BY company;
...
GROUP BY name
HAVING  SUM (salary)>2500--> After GROUP BY, WHERE cannot be used. We have to use HAVING to filter
--SELECT name, Sum (salary) AS total_salary
--FROM workers
--WHERE SUM(salary)  > 2500 --> WE cannot use aggregate functions with where clause
--GROUP BY naame;

--Find the number of employees if it is more than 1 per state
SELECT state,  COUNT(state) FROM workers
GROUP BY state
HAVING COUNT (state) >1--> HAVING is for filtering (if) for group by. After HAVING we have to useaggreagte function. Don't use field names
...
SELECT state,  MAX (salary) AS max_salary_for_every_state FROM workers
GROUP BY state
...
UNION ALL--> UNION ALL does same thing with UNION but it gives duplicate records
...
EXCEPT--> It is used to extract 1 query result from another query result. It returns unique records.
